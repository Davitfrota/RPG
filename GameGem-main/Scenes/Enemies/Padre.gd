...
#				animationPlayer.play("Run");
			...
			if(checkDistanceFromPlayer < 50):
				$DialogBox/DialogText.text = "HE KILLED OUR KING! \n KILL HIM!";
				if(!timerCreated):
				...
				if(waitToAttackTimer.time_left < 0.1):
					...
					$DialogBox/DialogText.text = "DONT KILL ME, PLEASE, \n I SWARE IM GOING TO CHANGE!";
...
func _on_Stats_no_health():
	...
	if(!PlayerStats.isNecromancer): #Player Não é Necromancer...
		...
		$DemonFormSprite.visible = true;
		$DemonFormSprite.play("DemonForm");
		Music.Demon_sound()
		$DialogBox/DialogText.text = "MY FATHER, ARTHUR MORAES, \n IS WAITING FOR YOU \n IN HELL!";
		yield($DemonFormSprite, "animation_finished");
		get_tree().change_scene("res://Videos & Audio/Video/VideoFim.tscn")
		queue_free()
